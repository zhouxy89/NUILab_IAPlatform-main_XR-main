...
Shader "IATK/SphereShader"
{
	Properties
	{
            ...
            _BrushedTexture("Base (RGB)", 2D) = "Yellow" {}
	...
	SubShader
	{
		Pass
		{
			...
			Zwrite On
			//ZTest LEqual
			//Tags{ "LightMode" = "ForwardBase" "Queue" = "Transparent" "RenderType" = "Transparent"} // "LightMode" = "ForwardBase" is the reason shader only show pinkness
			//ZTest Off
		    ...
		    SamplerState sampler_SpriteTex;
				...
				f_output FS_Main(g2f input): SV_Target0
				{
					...
					half3 n = tex2D(_BrushedTexture, input.tex0);
					n.x = (n.x - 0.5) / 0.5;
					n.y = -(n.y - 0.5) / 0.5;
					n.z = -(n.z - 0.5) / 0.5;
					...
					if (length(d) > 0.5 || input.color.w == 0) // change data point from cube to sphere
					{
					...
					else
					{
						//if (input.isBrushed > 0 && ShowBrush > 0.0)
							o.color = tex2D(_MainTex, dt) * BrushColor;
							//o.color = BrushColor;
							//o.color = tex2D(_MainTex, dt);
						
					  	//else	
                   				//o.color = BrushColor * tex2D(_BrushedTexture, input.tex0.xy);
