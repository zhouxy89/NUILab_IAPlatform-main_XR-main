...
public class RiggedHand : MonoBehaviour, IMixedRealityControllerVisualizer, IMixedRealitySourceStateHandler, IMixedRealityHandJointHandler
{
        ...
        IDictionary<TrackedHandJoint, MixedRealityPose> InputData = eventData.InputData;

            Transform jointTransform;
            // Apply updated TrackedHandJoint pose data to the assigned transforms
            foreach (TrackedHandJoint handJoint in InputData.Keys)
            {
                if (joints.TryGetValue(handJoint, out jointTransform))
                {
                    if (jointTransform != null)
                    {
                        if (handJoint == TrackedHandJoint.Palm)
                        {
                            ...
                            {
                                Palm.position = InputData[TrackedHandJoint.Wrist].Position;
                            }
                            else
                            {
                                Palm.position = InputData[TrackedHandJoint.Palm].Position;
                            }
                            Palm.rotation = InputData[TrackedHandJoint.Palm].Rotation * userBoneRotation;
                        }
                        else if (handJoint == TrackedHandJoint.Wrist)
                        {
                            if (!ModelPalmAtLeapWrist)
                            {
                                Wrist.position = InputData[TrackedHandJoint.Wrist].Position;
                                Wrist.position = InputData[TrackedHandJoint.Wrist].Position;
                            }
                        }
                        else
                        {
                        // Finger joints
                        jointTransform.rotation = InputData[handJoint].Rotation * Reorientation();

                        if (DeformPosition)
                            {
                                jointTransform.position = InputData[handJoint].Position;
                                ...
                                {
                                    ScaleFingerTip(InputData, jointTransform, handJoint + 1, jointTransform.position);
                                }
                            }
                        }
                    }
        ...
        private void ScaleFingerTip(IDictionary<TrackedHandJoint, MixedRealityPose> InputData, Transform jointTransform, TrackedHandJoint fingerTipJoint, Vector3 boneRootPos)
        {
            ...
            var boneTipPos = InputData[fingerTipJoint].Position;
